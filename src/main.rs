...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut switch_pin = Gpio::new()?.get(OUTPUT_SWITCH_PIN)?.into_output();
    // let mut status_pin = Gpio::new()?.get(INPUT_STATUS_PIN)?.into_input();

    // status_pin.set_async_interrupt(Trigger::Both, button_callback);

    let res = button_pin.set_async_interrupt(Trigger::Disabled, button_callback);
    match res {
        Ok(_) => (),
